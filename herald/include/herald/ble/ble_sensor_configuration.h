...
/// \brief Legacy Australia COVIDsafe service UUID. NOT detected by Herald for C++
static const UUID legacyAustraliaServiceUUID("00000000-0000-4000-8000-000000000000"); // TODO fill this value in
/// \brief Legacy OpenTrace service UUID. NOT detected by Herald for C++
static const UUID legacyOpenTraceUUID("A6BA4286-C550-4794-A888-9467EF0B31A8");
/// \brief Legacy Herald Service UUID prior to v2.1.0 in Jan 2023. NOT detected by Herald for C++
static const UUID legacyHeraldServiceUUID("428132af-4746-42d3-801e-4572d65bfd9b");
/// \brief Current Herald Linux Foundation Service UUID
static const UUID linuxFoundationServiceUUID("0000FCF6-0000-1000-8000-00805F9B34FB");

/// \brief Defines BLE sensor configuration data, e.g. service and characteristic UUIDs
struct BLESensorConfiguration {
  ...
  /// \brief Service UUID for beacon service. This is a fixed UUID to enable iOS devices to find each other even
  ...
  UUID serviceUUID;
  /// \brief Signaling characteristic for controlling connection between peripheral and central, e.g. keep each other from suspend state
  ...
  /// \brief BLE advert manufacturer ID for Apple, for scanning of background iOS devices
  int manufacturerIdForApple;

  // MARK:- BLE signal characteristic action codes

  /// \brief Signal characteristic action code for write payload, expect 1 byte action code followed by 2 byte little-endian Int16 integer value for payload data length, then payload data
  std::byte signalCharacteristicActionWritePayload;
  /// \brief Signal characteristic action code for write RSSI, expect 1 byte action code followed by 4 byte little-endian Int32 integer value for RSSI value
  std::byte signalCharacteristicActionWriteRSSI;
  /// \brief Signal characteristic action code for write payload, expect 1 byte action code followed by 2 byte little-endian Int16 integer value for payload sharing data length, then payload sharing data
  std::byte signalCharacteristicActionWritePayloadSharing;
  /// \brief Arbitrary immediate write
  ...
  /// \brief Payload update at regular intervals, in addition to default HERALD communication process.
  ...
  /// \brief Expiry time for shared payloads, to ensure only recently seen payloads are shared
  TimeInterval payloadSharingExpiryTimeInterval;

  /// \brief Advert refresh time interval
  TimeInterval advertRefreshTimeInterval;

  /// \brief Remove peripheral records that haven't been updated for some time.
  ...
  /// \brief Max connections - since v1.2 (allowing multiple connections on Android and C++)
  int maxBluetoothConnections; // Same as NRF 52840 max connections

  /// \brief Does this Herald application support advertising?
